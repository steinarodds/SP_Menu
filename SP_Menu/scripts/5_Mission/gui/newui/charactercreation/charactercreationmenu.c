modded class CharacterCreationMenu extends UIScriptedMenu
{
	...
	{
		layoutRoot = GetGame().GetWorkspace().CreateWidgets( "SP_Menu/gui/layouts/new_ui/character_creation/pc/character_creation.layout" );
	...
	override void ColorHighlight( Widget w )
	{
		...
		if( text1 )
		{
			text1.SetColor(UIColorText);
		};
		
		if( text2 )
		{
			text2.SetColor(UIColorText);
		};
		
		if( text3 )
		{
			text3.SetColor(UIColorText);
			w.SetAlpha(1);
		};
		
		if( image )
		{
			image.SetColor(UIColorText);
		};
		
		if ( option )
		{
			option.SetColor(UIColorText);
		};
	};
};
