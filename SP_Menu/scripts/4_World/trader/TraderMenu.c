...
modded class TraderMenu extends UIScriptedMenu
{
    ...
    {
		layoutRoot = GetGame().GetWorkspace().CreateWidgets( "SP_Menu/gui/layouts/trader/TraderMenu.layout" );
	...
	void updatePlayerCurrencyAmount()
	{
		...
		m_SaldoValue.SetText("Money: " + MakeNiceString(m_Player_CurrencyAmount));
	}

	string MakeNiceString(int m_Player_CurrencyAmount)
	{
	    string NiceString = "";
	    string OrginalString = m_Player_CurrencyAmount.ToString();
